...
export default function PupdateApp() {
  ...
  const hydrate = useCallback(async (active: Session) => {
    ...
    const [profiles, dogs, updates, photos, care] = await Promise.all([
      db<Profile[]>(current, "profiles", `?id=eq.${current.user.id}&select=*`),
      db<Dog[]>(current, "dogs", `?owner_id=eq.${current.user.id}&select=*`),
      db<Omit<Post, "photos">[]>(current, "pupdates", `?owner_id=eq.${current.user.id}&select=*&order=created_at.desc`),
      db<Photo[]>(current, "pupdate_photos", `?owner_id=eq.${current.user.id}&select=*&order=sort_order.asc`),
      db<CareEvent[]>(current, "care_events", `?owner_id=eq.${current.user.id}&select=*&order=occurred_at.desc`).catch(() => []),
    ]);
    ...
    setProfile(profiles[0] ?? null); setDog(dogs[0] ?? null);
    setPosts(updates.map(post => ({ ...post, photos: withUrls.filter(photo => photo.pupdate_id === post.id) })));
    setCareEvents(care);
    return current;
  }, []);
...
function Onboarding({ session, initialName, onDone }: { session: Session; initialName: string; onDone: () => void }) {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) { e.preventDefault(); setBusy(true); setError(""); const data = new FormData(e.currentTarget); let path: string | null = null; try { if (photo) path = await upload(session, "avatars", photo); await db(session, "profiles", "?on_conflict=id", { method: "POST", headers: { Prefer: "resolution=merge-duplicates,return=representation" }, body: JSON.stringify({ id: session.user.id, display_name: String(data.get("humanName")), onboarding_complete: true }) }); await db(session, "dogs", "", { method: "POST", body: JSON.stringify({ owner_id: session.user.id, name: String(data.get("name")), breed: String(data.get("breed")), birthday: String(data.get("birthday")), bio: String(data.get("bio")) || null, photo_path: path }) }); await onDone(); } catch (reason) { if (path) await removeUpload(session, "avatars", path).catch(() => undefined); setError(message(reason)); } finally { setBusy(false); } }
  return <main className="min-h-screen bg-[#fbf8f3] px-6 py-10"><form onSubmit={submit} className="mx-auto max-w-md"><Logo /><p className="mt-8 text-xs font-bold uppercase tracking-[.16em] text-[#8057ac]">One last thing</p><h1 className="mt-2 font-serif text-4xl font-bold">Create Pepper’s profile</h1><p className="mt-3 text-sm leading-6 text-[#7a6f7e]">This becomes the heart of your private family scrapbook.</p>{error && <p className="mt-5 rounded-2xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}<PhotoPicker file={photo} setFile={setPhoto} label="Add Pepper’s photo" /><div className="mt-6 space-y-4"><Field label="Your name"><input name="humanName" defaultValue={initialName} className="field" required /></Field><Field label="Pepper’s name"><input name="name" className="field" required /></Field><Field label="Breed"><input name="breed" className="field" required /></Field><Field label="Birthday"><input name="birthday" type="date" max={new Date().toISOString().slice(0, 10)} className="field" required /></Field><Field label="Bio (optional)"><textarea name="bio" className="field resize-none" rows={3} maxLength={500} /></Field></div><button disabled={busy} className="primary mt-6">{busy ? "Creating profile…" : "Start our scrapbook"}</button></form></main>;
}
...
function Feed({ posts, dog, profile, onCreate, onChanged, session, setError }: { posts: Post[]; dog: Dog; profile: Profile; onCreate: () => void; onChanged: () => void; session: Session; setError: (s: string) => void }) {
  ...
  return <main className="space-y-5 px-3 py-5 sm:px-6">{posts.map(post => <PostCard key={post.id} post={post} profile={profile} session={session} onChanged={onChanged} setError={setError} />)}</main>;
}

function PostCard({ post, profile, session, onChanged, setError }: { post: Post; profile: Profile; session: Session; onChanged: () => void; setError: (s: string) => void }) { const [index, setIndex] = useState(0); const [menu, setMenu] = useState(false); async function edit() { const caption = window.prompt("Edit caption", post.caption); if (caption === null) return; try { await db(session, "pupdates", `?id=eq.${post.id}`, { method: "PATCH", body: JSON.stringify({ caption, updated_at: new Date().toISOString() }) }); await onChanged(); } catch (reason) { setError(message(reason)); } } async function remove() { if (!window.confirm("Delete this Pupdate permanently?")) return; try { await Promise.all(post.photos.map(photo => removeUpload(session, "pupdates", photo.storage_path))); await db(session, "pupdates", `?id=eq.${post.id}`, { method: "DELETE" }); await onChanged(); } catch (reason) { setError(message(reason)); } } return <article className="overflow-hidden rounded-[26px] border border-black/[.05] bg-white shadow-[0_9px_30px_rgba(55,40,65,.06)]"><div className="flex items-center gap-3 px-4 py-3"><span className="grid size-10 place-items-center rounded-full bg-[#7450a8] text-sm font-bold text-white">{profile.display_name.slice(0, 1).toUpperCase()}</span><div className="min-w-0 flex-1"><p className="text-sm font-bold">{profile.display_name}</p><p className="text-[11px] text-[#8f8497]">{formatDate(post.created_at)}{post.location ? ` · ${post.location}` : ""}</p></div><div className="relative"><button onClick={() => setMenu(!menu)} className="grid size-9 place-items-center"><MoreIcon className="size-5" /></button>{menu && <div className="absolute right-0 z-10 w-32 rounded-xl bg-white p-1 text-xs font-bold shadow-xl"><button onClick={edit} className="w-full px-3 py-2 text-left">Edit</button><button onClick={remove} className="w-full px-3 py-2 text-left text-red-600">Delete</button></div>}</div></div>{post.photos[0]?.url && <div className="relative aspect-[4/5] bg-[#eee9e3]"><Image src={post.photos[index].url!} alt={post.caption || "Pupdate photo"} fill sizes="(max-width:680px) 100vw,680px" className="object-cover" />{index > 0 && <SlideButton left onClick={() => setIndex(index - 1)} />}{index < post.photos.length - 1 && <SlideButton onClick={() => setIndex(index + 1)} />}</div>}<div className="p-4"><p className="text-sm leading-6"><strong>{profile.display_name}</strong> {post.caption}</p>{post.tags.length > 0 && <p className="mt-2 text-[11px] font-bold uppercase tracking-wider text-[#8b62bd]">{post.tags.join(" · ")}</p>}</div></article>; }

function AddPupdate({ session, onCreated, setError }: { session: Session; onCreated: () => void; setError: (s: string) => void }) { const [files, setFiles] = useState<File[]>([]); const [busy, setBusy] = useState(false); async function submit(e: FormEvent<HTMLFormElement>) { e.preventDefault(); if (!files.length) return; setBusy(true); setError(""); const data = new FormData(e.currentTarget); const uploaded: string[] = []; try { for (const file of files) uploaded.push(await upload(session, "pupdates", file)); const [post] = await db<Post[]>(session, "pupdates", "", { method: "POST", body: JSON.stringify({ owner_id: session.user.id, caption: String(data.get("caption")), location: String(data.get("location")) || null, event_date: String(data.get("eventDate")) || null, tags: data.getAll("tags") }) }); await db(session, "pupdate_photos", "", { method: "POST", body: JSON.stringify(uploaded.map((storage_path, sort_order) => ({ owner_id: session.user.id, pupdate_id: post.id, storage_path, sort_order }))) }); await onCreated(); } catch (reason) { await Promise.all(uploaded.map(path => removeUpload(session, "pupdates", path).catch(() => undefined))); setError(message(reason)); } finally { setBusy(false); } } return <main className="px-5 py-6 sm:px-7"><h1 className="font-serif text-3xl font-bold">Add Pupdate</h1><form onSubmit={submit}><label className="mt-6 grid min-h-52 cursor-pointer place-items-center rounded-[28px] border-2 border-dashed border-[#c9b4df] bg-[#f2ecf7] p-6 text-center"><input className="sr-only" type="file" accept="image/*" multiple onChange={e => setFiles(Array.from(e.target.files ?? []))} /><span><CameraIcon className="mx-auto size-9 text-[#7450a8]" /><strong className="mt-3 block">Choose photos</strong><small className="mt-1 block text-[#887a90]">{files.length ? `${files.length} selected` : "Upload one or more moments"}</small></span></label><div className="mt-5 space-y-4"><Field label="Caption"><textarea name="caption" className="field resize-none" rows={4} maxLength={2200} /></Field><Field label="Location (optional)"><input name="location" className="field" /></Field><Field label="Event date (optional)"><input name="eventDate" type="date" className="field" /></Field><Field label="Private scrapbook tags"><div className="grid grid-cols-2 gap-2">{["walkies", "holiday", "treats", "with-friends"].map(tag => <label key={tag} className="rounded-full bg-white px-4 py-3 text-xs font-bold capitalize ring-1 ring-black/[.06]"><input type="checkbox" name="tags" value={tag} className="mr-2 accent-[#7450a8]" />{tag.replace("-", " ")}</label>)}</div></Field></div><button disabled={busy || !files.length} className="primary mt-6">{busy ? "Posting…" : "Post Pupdate"}</button></form></main>; }

function Scrapbook({ posts, dog }: { posts: Post[]; dog: Dog }) { const photos = posts.flatMap(post => post.photos.map(photo => ({ ...photo, date: post.event_date || post.created_at }))).sort((a, b) => b.date.localeCompare(a.date)); return <main className="px-4 py-6 sm:px-7"><h1 className="font-serif text-3xl font-bold">Scrapbook</h1><p className="mt-2 text-sm text-[#84798a]">Every Pupdate in one place!<br />{dog.name}, scrapbooked.</p>{photos.length ? <div className="mt-6 grid grid-cols-3 gap-1.5">{photos.map(photo => <div key={photo.id} className="relative aspect-square overflow-hidden rounded-xl bg-[#e8e1e9]">{photo.url && <Image src={photo.url} alt="Scrapbook memory" fill sizes="33vw" className="object-cover" />}</div>)}</div> : <EmptySmall text="Photos will appear here after your first Pupdate." />}</main>; }

function ProfilePage({ session, profile, dog, posts, onChanged, onSignOut, setError }: { session: Session; profile: Profile; dog: Dog; posts: Post[]; onChanged: () => void; onSignOut: () => void; setError: (s: string) => void }) { const [accountUrl, setAccountUrl] = useState<string | null>(null); const [dogUrl, setDogUrl] = useState<string | null>(null); useEffect(() => { void signedUrl(session, "avatars", profile.avatar_path).then(setAccountUrl); void signedUrl(session, "avatars", dog.photo_path).then(setDogUrl); }, [session, profile.avatar_path, dog.photo_path]); async function replace(kind: "user" | "dog", file?: File) { if (!file) return; try { const next = await upload(session, "avatars", file); const old = kind === "user" ? profile.avatar_path : dog.photo_path; await db(session, kind === "user" ? "profiles" : "dogs", kind === "user" ? `?id=eq.${session.user.id}` : `?id=eq.${dog.id}`, { method: "PATCH", body: JSON.stringify(kind === "user" ? { avatar_path: next } : { photo_path: next }) }); if (old) await removeUpload(session, "avatars", old); await onChanged(); } catch (reason) { setError(message(reason)); } } async function remove(kind: "user" | "dog") { const old = kind === "user" ? profile.avatar_path : dog.photo_path; if (!old) return; try { await db(session, kind === "user" ? "profiles" : "dogs", kind === "user" ? `?id=eq.${session.user.id}` : `?id=eq.${dog.id}`, { method: "PATCH", body: JSON.stringify(kind === "user" ? { avatar_path: null } : { photo_path: null }) }); await removeUpload(session, "avatars", old); await onChanged(); } catch (reason) { setError(message(reason)); } } return <main className="px-5 py-6 sm:px-7"><section className="rounded-[28px] bg-white p-5 shadow-sm"><h1 className="font-serif text-2xl font-bold">Your account</h1><PhotoManager url={accountUrl} label={profile.display_name} onChange={file => replace("user", file)} onDelete={() => remove("user")} /><p className="mt-3 text-sm text-[#7d7281]">{session.user.email}</p></section><section className="mt-5 overflow-hidden rounded-[28px] bg-white shadow-sm"><div className="relative h-56 bg-[#eee6f5]">{dogUrl ? <Image src={dogUrl} alt={dog.name} fill sizes="680px" className="object-cover" /> : <span className="grid h-full place-items-center text-7xl">🐾</span>}</div><div className="p-5"><h2 className="font-serif text-3xl font-bold">{dog.name}</h2><p className="mt-1 text-sm font-semibold">{dog.breed} · born {new Date(`${dog.birthday}T00:00:00`).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</p>{dog.bio && <p className="mt-3 text-sm leading-6 text-[#766b7a]">{dog.bio}</p>}<PhotoManager compact url={dogUrl} label={`${dog.name}’s photo`} onChange={file => replace("dog", file)} onDelete={() => remove("dog")} /></div></section><div className="mt-5 grid grid-cols-2 gap-3"><Stat value={posts.length} label="Pupdates" /><Stat value={posts.reduce((n, post) => n + post.photos.length, 0)} label="Memories" /></div><button onClick={onSignOut} className="secondary mt-7 text-red-600">Sign out</button></main>; }

function PhotoManager({ url, label, onChange, onDelete, compact = false }: { url: string | null; label: string; onChange: (f?: File) => void; onDelete: () => void; compact?: boolean }) { return <div className={`flex items-center gap-3 ${compact ? "mt-5" : "mt-4"}`}><div className="relative size-16 shrink-0 overflow-hidden rounded-full bg-[#eee6f5]">{url ? <Image src={url} alt={label} fill sizes="64px" className="object-cover" /> : <UserIcon className="m-4 size-8 text-[#7450a8]" />}</div><div className="flex min-h-16 flex-wrap items-center gap-2"><label className="inline-flex min-h-10 cursor-pointer items-center justify-center rounded-full bg-[#7450a8] px-4 py-2 text-center text-xs font-bold leading-none text-white"><input type="file" accept="image/*" className="sr-only" onChange={e => onChange(e.target.files?.[0])} />{url ? "Replace photo" : "Upload photo"}</label>{url && <button onClick={onDelete} className="inline-flex min-h-10 items-center justify-center rounded-full bg-red-50 px-4 py-2 text-xs font-bold leading-none text-red-600">Delete</button>}</div></div>; }
...
function PhotoPicker({ file, setFile, label }: { file: File | null; setFile: (f: File | null) => void; label: string }) { const preview = useMemo(() => file ? URL.createObjectURL(file) : null, [file]); return <label className="mx-auto mt-7 grid size-36 cursor-pointer place-items-center overflow-hidden rounded-full bg-[#eee6f5] text-center text-[#7450a8] ring-4 ring-white shadow-lg"><input type="file" accept="image/*" className="sr-only" onChange={e => setFile(e.target.files?.[0] ?? null)} />{preview ? <Image src={preview} alt="Photo preview" width={144} height={144} unoptimized className="h-full w-full object-cover" /> : <span><CameraIcon className="mx-auto size-7" /><small className="mt-2 block w-24 font-bold">{label}</small></span>}</label>; }
...
function UserProfile({ session, profile, onChanged, onSignOut, setError }: { session: Session; profile: Profile; onChanged: () => void; onSignOut: () => void; setError: (s: string) => void }) {
  ...
  async function saveName(e: FormEvent<HTMLFormElement>) { e.preventDefault(); const display_name = String(new FormData(e.currentTarget).get("displayName")); try { await db(session, "profiles", `?id=eq.${session.user.id}`, { method: "PATCH", body: JSON.stringify({ display_name, updated_at: new Date().toISOString() }) }); await onChanged(); } catch (reason) { setError(message(reason)); } }
  return <main className="px-5 py-6 sm:px-7"><p className="text-xs font-bold uppercase tracking-[.15em] text-[#8b62bd]">Your account</p><h1 className="mt-1 font-serif text-3xl font-bold">{profile.display_name}</h1><section className="mt-5 rounded-[28px] bg-white p-5 shadow-sm"><p className="text-xs font-bold uppercase tracking-[.12em] text-[#8b62bd]">Your profile photo</p><PhotoManager url={url} label={profile.display_name} onChange={replace} onDelete={remove} /><form onSubmit={saveName} className="mt-5"><Field label="Your display name"><div className="flex gap-2"><input name="displayName" defaultValue={profile.display_name} className="field min-w-0" required /><button className="rounded-full bg-[#eee6f5] px-4 text-xs font-bold text-[#67428f]">Save</button></div></Field></form><p className="mt-4 text-sm text-[#7d7281]">{session.user.email}</p></section><button onClick={onSignOut} className="secondary mt-7 text-red-600">Sign out</button></main>;
}
